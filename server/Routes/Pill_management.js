...
//-----------------------------------CreatePill------------------------------------
router.post('/CreatePillReminder',
    verifyToken,
    [
        body('pill_name')
            .notEmpty()
        ...
        body('reminder_times')
            .isArray({ min: 1 })
            .withMessage('Reminder times must be an array with at least one time slot'),
        body('reminder_times.*')
            .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
            .withMessage('Each reminder time must be in HH:MM:SS format ending with :00'),        
        body('NumberofPills')
            .isInt({ min: 1 })
    ...
    async (req, res) => {
        ...
        try {
            ...
            const createPillReminderQuery = `
                ...
                (pill_name, pill_note, pill_type, start_date, end_date, frequency, NumberofPills, pill_image, pill_Time, caregiver_id, elderly_id, status) 
                VALUES 
                (@pill_name, @pill_note, @pill_type, @start_date, @end_date, @frequency, @NumberofPills, @pill_image, @pill_Time, @caregiver_id, @elderly_id, 0);
            ...
            const createPillReminderResult = await createPillReminderRequest
                ...
                const PillReminder_id = createPillReminderResult.recordset[0].PillReminder_id;
                ...
                if (reminder_times && reminder_times.length > 0) {
                    const insertReminderTimesPromises = reminder_times.map(async (time) => {
                        ...
                        `;
                        try {
                            const result = await insertReminderTimeRequest
                            ...
                            console.error(`Failed to insert reminder time ${formattedTime}:`, err);
    ...
    router.get('/ShowPillRemailderListForCaregiver', verifyToken , async (req, res) => {
        try {
            ...
            const CaregiverPillList = await pool.request()
            .input('caregiver_id', sql.Int,id)
            .query('SELECT pill_name, pill_type, pill_note, frequency, pill_Time FROM CareYou.[Pill_Reminder] WHERE caregiver_id = @caregiver_id');
    ...
    router.get('/ShowPillRemindersListForElderlyPillBoxs', verifyToken, async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
            ...
            const elderlyPillList = await pool.request()
    ...
    router.get('/ShowTodayPillRemindersOfElderForCaregiverHome', verifyToken, async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
            ...
            if (CaregiverPillList.recordset.length > 0) {
                const PillList = CaregiverPillList.recordset.map(row => ({
                    ...
                    pill_note: row.pill_note,
                    pill_Time: row.pill_Time,
                    reminder_times: new Date(row.reminder_times).toISOString().split('T')[1].substring(0, 5)
                }));
                res.status(200).json(PillList);
            } 
        } catch (err) {
    ...
    router.get('/ShowTodayPillRemailderListForElderly', verifyToken , async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
    ...
    router.put('/EditPillReminder/:PillReminder_id',
    verifyToken,
    [
        body('pill_name')
            .optional()
        ...
        body('reminder_times.*')
            .optional()
            .matches(/^([01]\d|2[0-3]):([0-5]\d)$/)
            .withMessage('Each reminder time must be in HH:MM format ending with :00'),        
        body('NumberofPills')
            .optional()
            .isInt({ min: 1 })
            .withMessage('Number of pills must be a positive integer'),
    ...
    async (req, res) => {
        ...
        try {
            ...
            `;
            const pillReminderResult = await pool.request()
                .input('PillReminder_id', sql.Int, PillReminder_id)
            ...
            if (pill_name !== undefined) {
                ...
                updateParams.push({ name: 'pill_name', type: sql.VarChar, value: pill_name });
            } else {
                updateParams.push({ name: 'pill_name', type: sql.VarChar, value: existingPillReminder.pill_name });
            }

            if (pill_note !== undefined) {
                updatePillReminderQuery += 'pill_note = @pill_note, ';
                updateParams.push({ name: 'pill_note', type: sql.VarChar, value: pill_note });
            } else {
                updateParams.push({ name: 'pill_note', type: sql.VarChar, value: existingPillReminder.pill_note });
            }

            if (pill_type !== undefined) {
                updatePillReminderQuery += 'pill_type = @pill_type, ';
                updateParams.push({ name: 'pill_type', type: sql.NVarChar, value: pill_type });
            } else {
                updateParams.push({ name: 'pill_type', type: sql.NVarChar, value: existingPillReminder.pill_type });
            }

            if (start_date !== undefined) {
                updatePillReminderQuery += 'start_date = @start_date, ';
                updateParams.push({ name: 'start_date', type: sql.Date, value: start_date });
            } else {
                updateParams.push({ name: 'start_date', type: sql.Date, value: existingPillReminder.start_date });
            }

            if (end_date !== undefined) {
                updatePillReminderQuery += 'end_date = @end_date, ';
                updateParams.push({ name: 'end_date', type: sql.Date, value: end_date });
            } else {
                updateParams.push({ name: 'end_date', type: sql.Date, value: existingPillReminder.end_date });
            }

            if (frequency !== undefined) {
                updatePillReminderQuery += 'frequency = @frequency, ';
                updateParams.push({ name: 'frequency', type: sql.Int, value: frequency });
            } else {
                updateParams.push({ name: 'frequency', type: sql.Int, value: existingPillReminder.frequency });
            }

            if (NumberofPills !== undefined) {
                updatePillReminderQuery += 'NumberofPills = @NumberofPills, ';
                updateParams.push({ name: 'NumberofPills', type: sql.Int, value: NumberofPills });
            } else {
                updateParams.push({ name: 'NumberofPills', type: sql.Int, value: existingPillReminder.NumberofPills });
            }

            if (pill_image !== undefined) {
                updatePillReminderQuery += 'pill_image = @pill_image, ';
                updateParams.push({ name: 'pill_image', type: sql.VarChar, value: pill_image });
            } else {
                updateParams.push({ name: 'pill_image', type: sql.VarChar, value: existingPillReminder.pill_image });
            }

            if (pill_Time !== undefined) {
                updatePillReminderQuery += 'pill_Time = @pill_Time ';
                updateParams.push({ name: 'pill_Time', type: sql.NVarChar, value: pill_Time });
            } else {
                updateParams.push({ name: 'pill_Time', type: sql.NVarChar, value: existingPillReminder.pill_Time });
            }
    ...
    router.delete('/DeletePillReminder/:id', verifyToken, async (req, res) => {
        try {
            ...
            if (userRole !== 'Caregiver' && userRole !== 'Elderly' || userRole == 'Elderly') {
                return res.status(403).send('Unauthorized access');
            }
    
            // Ensure the pill reminder exists and is associated with the current user
            const PillReminderCheck = await pool.request()
                .input('PillReminder_id', sql.Int, PillReminder_id)
                .query('SELECT * FROM CareYou.[Pill_Reminder] WHERE PillReminder_id = @PillReminder_id');
            ...
            await pool.request()
                .input('PillReminder_id', sql.Int, PillReminder_id)
                .query('DELETE FROM CareYou.[PillReminder_Time] WHERE PillReminder_id = @PillReminder_id');
    
            // Delete the pill reminder
            await pool.request()
                .input('PillReminder_id', sql.Int, PillReminder_id)
                .query('DELETE FROM CareYou.[Pill_Reminder] WHERE PillReminder_id = @PillReminder_id');
