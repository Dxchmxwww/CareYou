...
router.post('/CreateAppointmentReminder',
    verifyToken,
    [
        body('Appointment_name').notEmpty().withMessage('Appointment name is required'),
        body('Date').isDate().withMessage('Date must be a valid date'),
        body('StartTime').matches(/^([01]\d|2[0-3]):([0-5]\d):00$/).withMessage('Start time must be in HH:MM format'),
        body('EndTime').matches(/^([01]\d|2[0-3]):([0-5]\d):00$/).withMessage('Start time must be in HH:MM format'),
    ...
    router.get('/ShowTodayAppointmentOfElderForCaregiverHome', verifyToken, async (req, res) => {
        try {
            ...
            const formattedAppointments = todayAppointmentsResult.recordset.map(appointment => ({
                Appointment_name: appointment.Appointment_name,
                StartTime: new Date(appointment.StartTime).toISOString().split('T')[1].substring(0, 5),
                EndTime: new Date(appointment.StartTime).toISOString().split('T')[1].substring(0, 5),
    ...
    router.get('/ShowTodayPillRemindersOfElderForCaregiver', verifyToken, async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
            ...
            const getTodayPillRemindersQuery = `
                ...
                WHERE caregiver_id = @caregiver_id AND date = @todayDate
            `;
            const todayPillRemindersResult = await pool.request()
                .input('caregiver_id', sql.Int, id)
                .input('todayDate', sql.Date, todayDate)
    ...
    router.get('/ShowAppointmentListForElderlyAppointmentBoxs', verifyToken, async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
            ...
            const elderlyAppointmentList = await pool.request()
            ...
            if (elderlyAppointmentList.recordset.length > 0) {
                const AppointmentList = elderlyAppointmentList.recordset.map(row => ({
                    ...
                    Date: row.Date,
                    StartTime: new Date(row.StartTime).toISOString().split('T')[1].substring(0, 5), // Format to HH:mm
                    EndTime: new Date(row.EndTime).toISOString().split('T')[1].substring(0, 5), // Format to HH:mm
    ...
    router.get('/ShowAllInfoAppointmentRemailderforCaregiver', verifyToken , async (req, res) => {
        try {
            const pool = await sql.connect(config);
            const id = req.user.id;
            ...
            const CaregiverAppointmentList = await pool.request()
            .input('caregiver_id', sql.Int,id)
            .query('SELECT Appointment_name, Date, StartTime, EndTime,Location FROM CareYou.[Appointment_reminder] WHERE caregiver_id = @caregiver_id');

            if (CaregiverAppointmentList.recordset.length > 0) {
                const AppointmentList = CaregiverAppointmentList.recordset.map(row => ({
                    Appointment_name: row.Appointment_name,
                    Date: row.Date,
    ...
    router.get('/ShowTodayAppointmentRemailderListForElderly', verifyToken, async (req, res) => {
        try {
            const id = req.user.id;
            const pool = await sql.connect(config);
    ...
    router.put('/EditAppointmentReminder/:Appointment_id',
    verifyToken,
    [
    ...
    async (req, res) => {
        ...
        try {
            ...
            `;
            const fetchAppointmentResult = await pool.request()
            ...
            if (Appointment_name !== undefined) {
                updateAppointmentQuery += 'Appointment_name = @Appointment_name, ';
                updateParams.push({ name: 'Appointment_name', type: sql.VarChar, value: Appointment_name });
            } else {
                updateParams.push({ name: 'Appointment_name', type: sql.VarChar, value: existingAppointment.Appointment_name });
            }

            if (Date !== undefined) {
                updateAppointmentQuery += 'Date = @Date, ';
                updateParams.push({ name: 'Date', type: sql.Date, value: Date });
            } else {
                updateParams.push({ name: 'Date', type: sql.Date, value: existingAppointment.Date });
            }

            if (StartTime !== undefined) {
                updateAppointmentQuery += 'StartTime = @StartTime, ';
                updateParams.push({ name: 'StartTime', type: sql.VarChar, value: StartTime });
            } else {
                updateParams.push({ name: 'StartTime', type: sql.VarChar, value: existingAppointment.StartTime });
            }

            if (EndTime !== undefined) {
                updateAppointmentQuery += 'EndTime = @EndTime, ';
                updateParams.push({ name: 'EndTime', type: sql.VarChar, value: EndTime });
            } else {
                updateParams.push({ name: 'EndTime', type: sql.VarChar, value: existingAppointment.EndTime });
            }

            if (Location !== undefined) {
                updateAppointmentQuery += 'Location = @Location ';
                updateParams.push({ name: 'Location', type: sql.VarChar, value: Location });
            } else {
                updateParams.push({ name: 'Location', type: sql.VarChar, value: existingAppointment.Location });
            }
            ...
            for (const param of updateParams) {
                updateAppointmentRequest.input(param.name, param.type, param.value);
            }
    ...
    router.delete('/DeleteAppointmentReminder/:Appointment_id', verifyToken, async (req, res) => {
        const { Appointment_id } = req.params;
        ...
        try {
            ...
            `;
            const checkAppointmentResult = await pool.request()
                .input('Appointment_id', sql.Int, Appointment_id)
                .input('id', sql.Int, id)
